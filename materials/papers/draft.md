...
従来、機械学習を用いた欠陥予測の研究は、ソースコードの構造的特徴を捉える静的メトリクスに依拠してきた。しかし、コードが絶えず変化する現代の開発現場においては、ソフトウェアの構造のみを反映する静的分析だけでは、構造の変化に関する特徴を十分に捉えきれず、予測精度の向上に課題がある。また、プロジェクトごとに異なる変更単位や変更頻度を考慮した分析手法も確立されていない。
...
第2章では、ソフトウェア品質と欠陥予測に関する関連研究を示す。静的メトリクス、変更メトリクス、および機械学習を用いた予測手法の動向を整理し、既存手法における時系列情報の扱いとレビュー労力の推定における課題を明確にする。
