...
5つのオープンソースプロジェクトを用いた実験により、提案手法の有効性を検証した。ランダムフォレストを用いた予測モデルにおいて、時系列変化を考慮することでF1スコアが向上することを確認した。また、Cost-Benefit Curveによる分析の結果、同じレビュー労力の下で従来手法よりも多くのバグを検出できることが示された。特に、Neo4jプロジェクトではレビュー労力40\%時点で17.0ポイント、Nettyプロジェクトでは20\%時点で19.8ポイントの改善が確認された。これにより、継続的な機能拡張が行われるソフトウェア開発において、効率的な品質保証活動を支援する実用的な手法を提供する。
...
第5章では実験結果を報告する。予測性能、特徴量重要度、Cost-Benefit Curveによる効果検証を示す。
...
本研究が特に注目するのは、\textbf{保守性}である。保守性は「意図した保守者によって、製品又はシステムが修正することができる有効性及び効率性の度合い」と定義され、5つの副特性を持つ。\textbf{モジュール性}（一つの構成要素に対する変更が他の構成要素に与える影響が最小になる度合い）、\textbf{再利用性}（一つ以上のシステムに資産を使用することができる度合い）、\textbf{解析性}（変更の影響を総合評価すること、欠陥若しくは故障の原因を診断すること、又は修正しなければならない部分を識別することが可能であることについての有効性及び効率性の度合い）、\textbf{修正性}（欠陥の取込みも既存の製品品質の低下もなく、有効的に、かつ、効率的に製品又はシステムを修正することができる度合い）、\textbf{試験性}（試験基準を確立し、その基準が満たされているかどうかを決定するために試験を実行することができる有効性及び効率性の度合い）である。

本研究では、コードの変更がこれらの品質特性、特に保守性の解析性・修正性にどのように影響を与えるかを分析する。コードの変化の過程を捉えることが重要である理由は、静的なスナップショットだけでは変更の勢いや不安定性を把握できないためである。例えば、ある時点でのコードメトリクスは問題なくても、短期間に頻繁な変更が繰り返されていれば、それは設計の不安定性や開発者の理解不足を示唆している可能性がある。時系列変化を分析することで、保守性を低下させる要因を明らかにすることを目指す。
...
## 2.1 コミット単位での欠陥予測の必要性
従来のソフトウェア欠陥予測手法では、ファイルやパッケージ単位での予測が主流であった。このアプローチでは、製品そのものの静的特性を測定する\textbf{コードメトリクス}（McCabeの循環的複雑度、CKメトリクス、コード行数など）や、開発プロセスから得られる履歴情報である\textbf{プロセスメトリクス}（過去の欠陥数、変更回数、開発者数など）を用いて、欠陥が発生しやすいモジュールを特定する。しかし、このアプローチには以下の3つの問題点がある。第一に、予測単位の粒度が大きいため、重要なファイルが特定された後も、開発者はレビューやテストの対象となる具体的な関数やコードの断片を見つけるのにかなりの時間を費やす必要があり、結果として品質保証活動の効率が低下する。第二に、予測がモジュール単位で行われるため、誰がその品質保証活動を担当すべきかが明確でなく、責任の所在が曖昧になることで対応の遅れや見逃しが発生しやすい。第三に、予測が開発サイクルの遅い段階で行われるため、問題が発見された時点では既に多くのコードが書かれており、欠陥が他のモジュールに波及している可能性があるため、修正コストが高くなる。これらの問題は、品質保証活動の実効性を低下させる。

これらの問題に対処するため、Kameiら\cite{kamei2013}は、ファイルやパッケージではなく、コミット単位での欠陥予測を行う「Just-In-Time品質保証」というアプローチを提案した。このアプローチでは、開発者がコードをリポジトリにコミットした直後に、その変更が欠陥を誘発するリスクを予測する。これにより、開発者は記憶が新しいうちにリスクの高い変更をレビューやテストできるため、より効率的かつ効果的な品質保証活動が可能になる。このアプローチは、JIS X 25010:2013で定義される保守性（特に解析性と修正性）の維持に貢献する。変更単位での予測には、予測単位の粒度が小さいこと、変更を行った開発者への具体的な作業割り当てとして予測を表現できること、開発サイクルの早い段階で予測が行われることという3つの利点がある。

Kameiらは、コード変更から抽出した特徴量を5つのカテゴリーに分類し、合計14個の変更メトリクスを提案した。このカテゴリー分類は、コード変更の多面的な特性を包括的に捉えるための理論的枠組みに基づいている。具体的には、変更の「空間的広がり」（Diffusion）、「量的規模」（Size）、「意図」（Purpose）、「過去の履歴」（History）、「開発者の能力」（Experience）という、変更に関する異なる側面を捉えることで、欠陥リスクを多角的に評価することが可能になる。これらのメトリクスは以下の通りである。

\begin{itemize}
    \item Diffusion
    \begin{itemize}
        \item 変更の拡散範囲を測定するメトリクスであり、NS（変更されたサブシステムの数）、ND（変更されたディレクトリの数）、NF（変更されたファイルの数）、変更の広がりが含まれる。広範囲に分散した変更は欠陥発生リスクが高いと考えられる。この理由は、複数の箇所に分散した変更により開発者の注意力が分散し、変更箇所間の相互作用や副作用を見落としやすくなるためである。また、変更が広範囲に及ぶほど、統合テストの複雑度が増し、全ての変更箇所の整合性を確認することが困難になる。さらに、複数のサブシステムやディレクトリにまたがる変更は、各部分の実装の理解が不完全になりやすく、結果として欠陥混入のリスクが高まる。
    \end{itemize}
    \item Size
    \begin{itemize}
...
しかし、この手法には重要な単純化が含まれている。レビュー労力を「変更された行の総数」のみで計算しているため、変更の複雑さや影響範囲の広さが考慮されていない。Kameiらがこの単純化を採用した理由は、初期研究として計算の容易性を優先し、また変更行数というメトリクスがバージョン管理システムから容易に取得できる客観的な指標であるためと考えられる。しかし、実際のレビュー労力は、変更されたファイル数や変更の広がりといった要因にも依存する。例えば、10行の変更でも、それが10個の異なるファイルに分散していれば、各ファイルのコンテキストを理解する必要があり、単一ファイルの10行変更よりも労力がかかる。このため、Kameiらの手法ではこれらの要因が労力計算に反映されていないという限界がある。

特徴量の重要性を分析した結果、OSSプロジェクトではNF、LA/LT、LT/NF、FIXが、商用プロジェクトではNF、LA/LT、LT/NF、NDEV、AGEが、欠陥リスクを増加させる最も重要な要因であることが明らかになった。ただし、これらのメトリクスにはいくつかの問題点が存在する。

FIXは特定のコミットが欠陥修正であるかどうかを示すフラグであり、どのメソッドやクラスが欠陥を誘発しやすいかという具体的な情報を提供しない。AGEは商用プロジェクトでは有効だが、OSSプロジェクトでは効果的ではない。この違いが生じる理由は、商用プロジェクトでは定常的な開発体制により変更間隔が比較的規則的であるのに対し、OSSプロジェクトではボランティアベースで開発が行われ、個々の開発者の都合により開発頻度が不定期になるためである。NDEVも同様に、商用プロジェクトではチームでの共同作業が多いため複数の開発者が同一ファイルを変更することが一般的であり有効だが、OSSプロジェクトでは1つの変更は基本的に1人の開発者が担当するため、NDEV値のバリエーションが少なく効果的ではない。NSはサブシステムの総数がプロジェクトによって異なるため、プロジェクト間での比較が困難である。これは、あるプロジェクトで3サブシステムに変更が及ぶことと、別のプロジェクトで3サブシステムに変更が及ぶことの意味が、プロジェクトのサブシステム総数によって大きく異なるためである。

## 2.2 コミット時点でのコード特性の把握と課題
...
従来の方法が不十分である理由は、リリース間の時間間隔が長いため、その間に多数の変更が発生し、どの変更が実際に欠陥を誘発したかという因果関係が不明確になるためである。例えば、リリースAで正常だったコードがリリースBで欠陥を持っていた場合、その間の数十から数百のコミットのうち、どれが欠陥を混入させたかを特定することは困難である。Ferencらのアプローチでは、SZZアルゴリズムにより欠陥を混入させたコミットを特定することで、欠陥との因果関係をより明確に捉えることができる。
...
これらの結果は、より細かい粒度（メソッドレベル）での予測が、より高い精度を達成できることを示している。細かい粒度で精度が向上する理由は、以下の3点にある。第一に、メソッドレベルでは局所的な変更特性を正確に捕捉できるため、欠陥との因果関係がより明確になる。ファイルやクラスレベルでは、欠陥を含まないメソッドと欠陥を含むメソッドが混在し、特徴量がぼやけてしまう。第二に、粒度が細かいほど、ノイズとなる無関係な情報が削減される。例えば、ファイル内の1つのメソッドにのみ欠陥がある場合、ファイルレベルの特徴量にはその欠陥と無関係な他のメソッドの情報も含まれてしまう。第三に、メソッドレベルでは各特徴量の意味が明確であり、機械学習モデルが欠陥パターンを学習しやすくなる。

一方、Hanらは、コードレビューを通じた欠陥検出の実態を調査した。OpenStackプロジェクトのNovaとNeutronを対象に、19,146件のレビューコメントを手動で分析し、将来の不具合を招く恐れのある不適切な構造がどの程度特定されるかを調査した。こうした不適切な構造とは、Martin Fowler が提唱した概念であり、コード内の潜在的な問題を示す特徴的なパターン（例: 肥大化したメソッド、重複コード、過度に複雑なクラス構造など）を指す。これらは必ずしも欠陥ではないが、保守性を低下させ、将来的に欠陥を誘発しやすくする要因となる。

その結果、コードレビューでこうした不適切な構造が特定されることは一般的ではないことが明らかになった。約1,200件のレビューのうち、不適切な構造が明示的に指摘されたのは限られた数であった。レビューで問題が指摘されない理由として、以下の要因が考えられる。第一に、レビュアーの時間的制約により、不適切な構造のような潜在的な問題よりも、明らかなバグや機能の正しさの検証に時間が割かれる。第二に、不適切な構造の認識には高度な設計知識が必要であり、全てのレビュアーがそのような知識を持つわけではない。第三に、不適切な構造の指摘はコードの大幅な書き直しを要求することになり、開発者との関係性やプロジェクトのスケジュールへの影響を考慮して、指摘を控える場合がある。

さらに、レビューの大部分（70\%）では、特定された不適切な構造について説明が提供されておらず、レビュアーは単に問題を指摘するだけで、その理由を詳しく説明していなかった。説明が提供されない理由として、レビューコメントの記述にかかる時間的コストや、問題点が自明であるとレビュアーが考えている可能性、あるいは説明する適切な言葉を見つけることの困難さなどが考えられる。

これらの研究から、コミットベースの欠陥予測における重要な課題が明らかになる。第一に、Ferencらの研究では各コミットの特性値を用いた予測が行われているが、コミット間の変化は対象外である。すなわち、あるコミット時点でのコードメトリクスは測定されているが、直前のコミットからどのように変化したかという時系列情報は活用されていない。第二に、Hanらの研究が示すように、コードレビューでは70\%のケースで欠陥の原因が明示されないため、レビューテキストのみから欠陥を予測することには限界がある。

これらの課題を解決することが重要である理由は、以下の通りである。コミット間の変化を考慮することで、コードの不安定性や急激な複雑化といった動的なリスク要因を捉えることができ、予測精度の向上が期待できる。また、レビューテキストへの依存を減らし、定量的なメトリクスベースのアプローチを採用することで、より客観的で自動化可能な欠陥予測システムを構築できる。これらの課題は、特性値同士の関連性を分析する新たなアプローチの必要性を示唆している。

## 2.3 リアルタイム品質検査の実現と課題
...
提案手法の効果を評価するため、Cost-Benefit Curveを用いる。これは、横軸に投入したレビュー労力、縦軸に発見したバグ数をプロットしたグラフである。
...
本研究において、ソフトウェアテスト研究で広く用いられる Defects4J\cite{just2014}ではなく、BugHunter Datasetを採用した理由は以下の2点に集約される。第一に、バグのライフサイクルにおける観測範囲の違いである。Defects4Jは「バグ修正の直前・直後（$V_{bug}$ と $V_{fix}$）」の状態を高度に隔離して提供するデータセットであり、バグがいつ、どのような変更によって混入したかという情報は欠落している。これに対し、BugHunter Datasetはバグ混入コミットとバグ修正コミットをペアで特定しており、バグの発生から収束までを捉えている。これにより、本研究の核となる「同一メソッド内での時系列的な変化量（変更メトリクス）」の算出が可能となる。第二に、分析の粒度と目的の適合性である。Defects4Jは主にテスト実行や自動バグ修正を目的としており、ファイルやクラス単位での管理が主である。一方、BugHunter Datasetは最初からメソッドレベルでの豊富なコードメトリクスを提供しており、本研究が目的とする「メソッド単位の緻密な欠陥予測」を、前処理のコストを抑えつつ直接実施できる利点がある。
...
\textbf{ステップ1（ベースライン）}: 変更メトリクスを追加する前のモデル。BugHunter Datasetに元々含まれているコードメトリクスのみを特徴量として使用する。このモデルをベースラインとし、後続のモデルとの比較基準とする。
...
\textbf{特定労力時点でのバグ発見数}: 総労力の20\%、40\%、60\%時点で発見できたバグ数を比較する。

これらの時点を評価対象とした理由は、実際の開発現場におけるレビュー労力の制約を反映するためである。第一に、20\%時点は非常に限られたレビューリソースしか利用できない状況を想定している。プロジェクトの初期段階や、緊急リリース前など、時間的制約が厳しい場合でも、最も重要な箇所を効率的に特定できるかを評価する。第二に、40\%時点は通常のレビュープロセスで現実的に投入可能な労力を想定している。多くの開発チームでは、全てのコミットを詳細にレビューする時間がなく、実際には変更の40〜50\%程度がレビュー対象となることが多い。第三に、60\%時点は比較的余裕のあるレビュープロセスを想定しており、提案手法の長期的な効果を評価する。これらの段階的評価により、異なるリソース制約下での提案手法の有効性を多角的に検証できる。
...
表\ref{tab:evaluation}に、5つのプロジェクトにおける各段階のF1スコア、適合率、再現率、正解率、ROC-AUCを示す。
...
## 6.2 変更規模と欠陥混入確率の関係
第5章のPartial Dependence Plot分析で観察された「変更規模が小さいほど欠陥混入確率が高い」という傾向について考察する。この現象は全5プロジェクトで一貫して確認されており、偶然ではなく何らかの本質的な要因が存在すると考えられる。
