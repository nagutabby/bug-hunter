...
## 4.1 汎用性検証のための対象選定
\paragraph{データセットの選定}
本研究では、Ferencらが構築したBugHunterデータセット\cite{ferenc2020}を基盤として使用する。このデータセットは、GitHub\cite{github}でホストされている15のJavaプロジェクトから自動的に収集されたバグ情報と、各コミットにおけるソースコード要素（ファイル、クラス、メソッド）のメトリクスを含んでいる。

本研究において、ソフトウェアテスト研究で広く用いられる Defects4J\cite{just2014}ではなく、BugHunterデータセットを採用した理由は以下の2点に集約される。第一に、バグのライフサイクルにおける観測範囲の違いである。Defects4Jは「バグ修正の直前・直後（$V_{bug}$ と $V_{fix}$）」の状態を高度に隔離して提供するデータセットであり、バグがいつ、どのような変更によって混入したかという情報は欠落している。これに対し、BugHunterデータセットはバグ混入コミットとバグ修正コミットをペアで特定しており、バグの発生から収束までを捉えている。これにより、本研究の核となる「同一メソッド内での時系列的な変化量（変更メトリクス）」の算出が可能となる。第二に、分析の粒度と目的の適合性である。Defects4Jは主にテスト実行や自動バグ修正を目的としており、ファイルやクラス単位での管理が主である。一方、BugHunterデータセットは最初からメソッドレベルでの豊富なコードメトリクスを提供しており、本研究が目的とする「メソッド単位の緻密な欠陥予測」を、前処理のコストを抑えつつ直接実施できる利点がある。
