...
## 2.2 静的メトリクスに基づく欠陥予測
構造的メトリクスは、ソースコードの構造の特徴を表すものである。代表的なメトリクスとして、複雑度メトリクス、規模メトリクス、結合度・凝集度メトリクスがある。
...
構造的メトリクスは、ある時点でのコードのスナップショットのみを分析するため、変更の頻度やパターンといった動的な情報を捉えられない。変更の頻度やパターンは構造的メトリクスよりも欠陥混入との関連性が高いことが知られており\cite{graves2000}、構造的メトリクスだけでは予測精度に限界がある。
