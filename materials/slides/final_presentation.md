...
### ToDo: 不均衡データがどのようなものかが分かるような図を追加
* 欠陥混入データの割合が小さいデータセットほど、提案手法による改善効果が大きい
* 変更メトリクスを用いることで、不均衡度が高いデータからも「欠陥を含む変更の特徴」を抽出できるようになる
* 時系列変化情報の導入により、データの偏りに左右されず、欠陥混入の兆候を捉えやすくなる

## 考察: 変更規模と欠陥混入確率の関連
### ToDo: 変更規模が小さいほど欠陥混入確率が高いことを示す図を追加
* 変更規模が小さいほど欠陥混入確率が高いという傾向が観察された
* 要因の考察:
  1. 小規模な変更は暫定的なバグ修正であることが多く、新たな不具合を誘発しやすい<sup>[5]</sup>
