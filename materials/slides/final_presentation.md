...
### ToDo: 反復的な開発手法の全体像とそれに伴う欠陥の波及を表す図を挿入
* 段階的・反復的な開発手法の普及:
  リリース後もソースコードの継続的かつ頻繁な変更が行われる
* レビュー効果を高めることが要求される一方、以下の2つはトレードオフの関係にある:
  - レビュー労力を下げる
  - 欠陥混入リスクを下げる
* この均衡点を求めるには、レビューの際の意思決定プロセスやモデルが必要
...
* 全体的視点（コミット単位）: 変更ファイル数、追加・削除行数、およびエントロピーに基づく変更の分散度を使用
...
### ToDo: 反復的な開発手法の全体像とそれに伴う欠陥の特徴の変化を表す図を挿入
...
* コミット間の不規則な変化を捉えることで、従来の静的な解析と比較して精度の高い予測を実現
* 労力を考慮したレビュー優先度付けにより、現実の開発環境において品質保証を効率化するモデルを提案
