...
* 欠陥発見効率の高いレビュー対象の選定
  - 補正済みレビュー労力: 変更の規模と分散度を活用し、対数変換により巨大な変更の影響を緩和した指標を定義
    ...
    - 貪欲法による求解: 単位労力あたりの欠陥混入確率が高い順に選定
...
### ToDo: 何についての考察かが分かるような図を追加
* 変更メトリクスの有効性:
  - 構造的メトリクス（例: 複雑度や行数）のみでは予測が困難なプロジェクトほど、時系列変化情報の追加による改善幅が大きくなる傾向を確認
* 汎用性の検証:
  - 検索エンジン、データベース、通信プロトコルなどの異なるドメインに対し、高い予測性能を発揮
* 提案手法は特定のプロジェクト構造に依存せず、コード構造の変化を捉えることで広範な適用が可能

## 考察: 不均衡データにおける予測モデルの挙動
### ToDo: 何についての考察かが分かるような図を追加
### 不均衡データがどのようなものかが分かるような図を追加
* 不均衡データへの対応:
  - 陽性クラス（欠陥あり）の割合が低いデータセットほど、提案手法による改善効果が大きい
* 識別能の向上:
  - 変更メトリクスを用いることで、不均衡度が高いデータからも「欠陥を含むコミット・メソッドの特徴」を抽出できるようになる
* 時系列変化情報の導入により、データの偏りに左右されず、欠陥混入の兆候を捉えやすくなった

## 考察: 変更規模と欠陥混入確率の関連
### ToDo: 何についての考察かが分かるような図を追加
* PDP分析により、変更規模が小さいほど欠陥混入確率が高いという傾向が観察された
* 要因の考察:
  1. 小規模な変更は部分的なバグ修正であることが多く、新たな不具合を誘発しやすい<sup>[5]</sup>
  2. 小規模な変更はレビューが容易なため、潜在的な欠陥が発見されやすく、記録に残る可能性が高い（欠陥密度 ≠ 欠陥混入数）
